...
namespace BizzyBeeGames
{
	public class UIMonoBehaviour : MonoBehaviour
	{
		...
		public void SinglePulse(Vector2 origScale, float pulseForce, float pulseAnimDuration)
		{
			Vector2 pulseScale = origScale * pulseForce;

			transform.localScale = pulseScale;

			UIAnimation anim;

			anim = UIAnimation.ScaleX(transform as RectTransform, pulseScale.x, origScale.x, pulseAnimDuration);
			anim.style = UIAnimation.Style.EaseOut;
			anim.Play();
			
			anim = UIAnimation.ScaleY(transform as RectTransform, pulseScale.y, origScale.y, pulseAnimDuration);
			anim.style = UIAnimation.Style.EaseOut;
			anim.Play();
		}
