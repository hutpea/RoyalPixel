...
public static class TextureUtilities
{
    ...
    public static List<PaletteItem> Pixelize(Color[] colors, int width, int height, Texture2D outTexture, int scaleToWidth, int scaleToHeight, ScaleType scaleType, float mergeThreshold, out bool firstItemIsBlank)
    {
        float xScale = (float)width / (float)scaleToWidth;
        float yScale = (float)height / (float)scaleToHeight;

        List<PaletteItem> palette = new List<PaletteItem>();

        PaletteItem blankPaletteItem = null;
        ...
        for (int x = 0; x < scaleToWidth; x++)
        {
            for (int y = 0; y < scaleToHeight; y++)
            {
                ...
                switch (scaleType)
                {
                    case ScaleType.CenterPixel:
                        int xCenter = Mathf.FloorToInt((float)x * xScale + xScale / 2f);
                        int yCenter = Mathf.FloorToInt((float)y * yScale + yScale / 2f);
                        int index = yCenter * width + xCenter;

                        color = colors[index];

                        break;
                    case ScaleType.BoxSampling:
                        ...
                        color = GetColorAverage(colors, width, height, xStart, yStart, xEnd - xStart, yEnd - yStart);
                        break;
                }

                if (color.a == 0f)
                {
                    if (blankPaletteItem == null)
                    {
                        blankPaletteItem = new PaletteItem(Color.white, x, y);
                    }
                    else
                    {
                        blankPaletteItem.AddCoords(x, y);
                    }
                }
                else
                {
                    ...
                    AddColorToPalette(color, x, y, palette, mergeThreshold);
                }
            }
        }
        ...
        if (blankPaletteItem != null)
        {
            palette.Insert(0, blankPaletteItem);

            firstItemIsBlank = true;
        }
        else
        {
            firstItemIsBlank = false;
        }

        ApplyPaletteToTexture(outTexture, palette);
    ...
    public static Color GetColorAverage(Color[] colors, int width, int height, int xStart, int yStart, int blockWidth, int blockHeight)
    {
        ...
        return new Color(r / num, g / num, b / num, allAlpha ? 0f : 1f);
    }
    ...
    public static string ExportTextureToFile(
        ...
        int awardAmount = 0,
        string nameCate = "",
        PictureType pictureType = PictureType.Free,
        int numberAds = 0,
        bool newPic = false
       )
    {
        ...
        if (nameCate != "")
        {
            contents += nameCate + "," + (newPic ? "NEW" : "") + "\n";
            contents += pictureType.ToString() + (numberAds != 0 ? "," + numberAds + "\n" : "\n");
        }
        // Add level lock info
        if (nameCate == "")
        {
        ...
        for (int i = 0; i < palette.Count; i++)
        {
            ...
            string rStr = paletteItem.color.r.ToString(System.Globalization.CultureInfo.InvariantCulture);
            string gStr = paletteItem.color.g.ToString(System.Globalization.CultureInfo.InvariantCulture);
            string bStr = paletteItem.color.b.ToString(System.Globalization.CultureInfo.InvariantCulture);

            contents += string.Format("{0},{1},{2}", rStr, gStr, bStr);
        }
        ...
        string filepath = string.Format("{0}/{1}.csv", outputFolderPath, id);
