...
namespace UnityEngine.UI.Extensions
{
    ...
    public class HorizontalScrollSnap : ScrollSnapBase, IEndDragHandler
    {
        ...
        void Update()
        {
            if (!_lerp && _scroll_rect.velocity == Vector2.zero)
            {
                if (!_settled && !_pointerDown)
                {
                    ...
                    {
                        if (CurrentPage == 0 || CurrentPage == _screens - 1)
            ...
            else if (_lerp)
            {
                ...
                if (Mathf.Abs(_screensContainer.localPosition.x - _lerp_target.x) < 0.1f)
                {
        ...
        public void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (_scroll_rect.horizontal)
            {
                ...
                if ((UseFastSwipe && distance < panelDimensions.width && distance >= FastSwipeThreshold) || UseHardSwipe)
                {
                    ...
                    {
                        if (_startPosition.x - _screensContainer.localPosition.x < _childSize / 4 && !IsMove(SwipeVelocityThreshold))
                        {
        ...
        {
            return (_scroll_rect.velocity.x > v) ||
                                (_scroll_rect.velocity.x < -v);
        }
